...
Yes using optimization flags can impact reproducibility as reordering of operations can cause a round off error accumulation. However, this effect does not impact the general convergence of the algorithm, i.e. we expect to converge to the same number given infinite precision. With that in mind and considering how high precision is not very expensive relative to low precision (we established that in section 3.), one can make the statment that using the flags for performance gain is indeed "worth it" as Solutions themselves will be in a very similar range. The only time one might hold off on them is if one plans to do bitwise comparisons of algorithms (i.e. see if two algorithms produce the exact same result).